...
# Vendored from https://github.com/tauri-apps/linuxdeploy-plugin-gtk
# with a patch to handle statically-linked binaries in AppDir/usr/bin/.
#
# Tauri's pinned linuxdeploy crashes when `ldd` returns exit code 1 on
# static binaries (like bun). The patch moves them out before linuxdeploy
# scans usr/bin/, then restores them after.
#
